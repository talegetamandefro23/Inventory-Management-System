...
export default function NewRequisition() {
  ...
  const [lines, setLines] = useState<LineItem[]>([
    { id: 1, name: "High-Performance Server Rack", sku: "SK-IT-203-01", qty: 2, unit: "EA", price: 1250 },
    { id: 2, name: "Cat6 Networking Cables (100m)", sku: "SK-IT-203-02", qty: 5, unit: "Roll", price: 85.5 },
  ]);
  ...
  function addLine() {
    setLines((prev) => [...prev, { id: Date.now(), name: "", sku: "", qty: 1, unit: "EA", price: 0 }]);
  }
  ...
  function validate() {
    ...
    if (lines.some((l) => !l.name.trim())) errs.push("All item names must be filled in.");
    return errs;
  }
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        <div className="h-14 w-14 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 text-2xl">✓</div>
        <p className="font-semibold text-lg">Requisition Submitted!</p>
        <p className="text-sm text-zinc-500">Routing to approval queue… redirecting to list.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        trail={["Procurement", "Purchase Requisitions", "Create New Requisition"]}
      ...
      <Card title="Expected Approval Route" className="mb-5">
        <div className="flex items-center gap-4">
          {[["John Doe", "Initiator"], ["Sarah Miller", "Dept. Head"], ["Robert Chen", "Finance Dir."]].map(([name, role], i) => (
            <div key={role} className="flex items-center gap-4 flex-1">
              ...
              {i < 2 && <div className="flex-1 h-px border-t border-dashed border-zinc-300" />}
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 space-y-5">
          <Card title="General Information" subtitle="Header details for this requisition.">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-xs text-zinc-500">Requesting Department</label>
                <input value={department} onChange={(e) => setDepartment(e.target.value)} className="w-full mt-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={project} onChange={(e) => setProject(e.target.value)} className="w-full mt-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm" placeholder="Project ARES: Automation" />
              </div>
              <div>
                <label className="text-xs text-zinc-500">Desired Delivery Date</label>
                <input type="date" className="w-full mt-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm" />
              </div>
            </div>
          </Card>
              ...
              <tbody>
                {lines.map((it) => (
                  <tr key={it.id} className="border-b border-zinc-50">
                    <td className="py-2">
                      <input value={it.name} onChange={(e) => updateLine(it.id, "name", e.target.value)} className="w-full border border-zinc-100 rounded px-2 py-1 text-sm" placeholder="Item name" />
                      <input value={it.sku} onChange={(e) => updateLine(it.id, "sku", e.target.value)} className="w-full border-0 text-xs text-zinc-400 px-2 py-0 mt-0.5 outline-none" placeholder="SKU" />
                    </td>
                    <td className="py-2">
                      <input type="number" value={it.qty} onChange={(e) => updateLine(it.id, "qty", Number(e.target.value))} className="w-full border border-zinc-100 rounded px-2 py-1 text-sm text-center" min={1} />
                    </td>
                    <td className="py-2">
                      <input value={it.unit} onChange={(e) => updateLine(it.id, "unit", e.target.value)} className="w-full border border-zinc-100 rounded px-2 py-1 text-sm" />
                    </td>
                    <td className="py-2">
                      <input type="number" value={it.price} onChange={(e) => updateLine(it.id, "price", Number(e.target.value))} className="w-full border border-zinc-100 rounded px-2 py-1 text-sm text-right" step={0.01} />
                    </td>
                    <td className="py-2 font-medium text-right pr-2">${(it.qty * it.price).toFixed(2)}</td>
                    <td className="py-2">
                      <button onClick={() => removeLine(it.id)} className="text-zinc-300 hover:text-red-500">
                        <Trash2 size={13} />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="space-y-5">
          <Card title="Supporting Documents">
            ...
            {["Supplier_Quote_A.pdf", "System_Specs_Final.docx"].map((f) => (
              <div key={f} className="flex items-center gap-2 text-xs border border-zinc-100 rounded-lg px-3 py-2 mb-2">
          ...
          <Card title="Suggested Suppliers" subtitle="Based on item category history.">
            {[
              ["TechSolutions Global", "4.8", "Lowest Price", true],
              ["OmniNet Systems", "4.5", "Fastest Lead Time", false],
              ["Logix Hardware", "4.2", "Local", false],
            ].map(([n, r, tag, pref]) => (
              <div key={n as string} className="rounded-lg border border-zinc-100 p-2.5 mb-2 text-sm">
          ...
          {needsMultipleQuotes && (
            <div className="bg-amber-50 text-amber-700 text-xs rounded-lg p-3">
              ...
              Total value exceeds $2,000. Three comparative quotes are required before Finance Director approval.
            </div>
          )}
